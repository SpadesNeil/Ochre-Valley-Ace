...
/obj/effect/proc_holder/spell/invoked/headbutt/cast(list/targets, mob/living/user,)
	...
	if(user.IsKnockdown()) // can't do it while on the floor.
		...
		animate(user, pixel_x = original_user_pixel_x, pixel_y = original_user_pixel_y, time = 1 SECONDS) // reset animation //OV Edit - Removed user transform
		animate(target, pixel_x = original_target_pixel_x, pixel_y = original_target_pixel_y, time = 1 SECONDS) // reset animation //OV Edit - Removed target transform
		revert_cast()
