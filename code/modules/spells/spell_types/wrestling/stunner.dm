...
/obj/effect/proc_holder/spell/invoked/stunner/cast(list/targets, mob/living/user,)
	...
	//var/original_user_transform = user.transform //OV Edit - Size Multipler Conflict Removal
	var/original_target_pixel_z = target.pixel_z
	//var/original_target_transform = target.transform //OV Edit - Size Multipler Conflict Removal
	...
	var/drop_timer = addtimer(CALLBACK(src, PROC_REF(drop_both), user, target, original_user_pixel_z, original_target_pixel_z), 5, TIMER_STOPPABLE) //OV Edit - user & target transform removed
	...
	if(user.IsKnockdown()) // can't do it while on the floor.
		...
		deltimer(drop_timer)
		animate(user, pixel_x = original_user_pixel_x, pixel_y = original_user_pixel_y, pixel_z = original_user_pixel_z, time = 1 SECONDS) // reset animation //OV Edit - user transform removed
		animate(target, pixel_z = original_target_pixel_z, time = 1 SECONDS) // reset animation //OV Edit - target transform removed
		revert_cast()
...
/obj/effect/proc_holder/spell/invoked/stunner/proc/drop_both(mob/living/user, mob/living/target, original_user_pixel_x, original_user_pixel_y, original_user_pixel_z, original_target_pixel_z) //OV Edit - user and target transform removed
	animate(user, pixel_x = original_user_pixel_x, pixel_y = original_user_pixel_y, pixel_z = original_user_pixel_z, time = 5) //OV Edit - User Transform Removed
	animate(target, pixel_z = original_target_pixel_z, time = 5) //OV Edit - Target Transform Removed
